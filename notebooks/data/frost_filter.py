...
def compute_coef_var(image, x_start, x_end, y_start, y_end):
    ...
    """
    if x_start < 0:
        raise ValueError("ERROR: x_start must be >= 0.")
    if y_start < 0:
        raise ValueError("ERROR: y_start must be >= 0.")
    ...
    if x_overflow:
        raise ValueError("ERROR: invalid parameters cause x window overflow.")
    if y_overflow:
        raise ValueError("ERROR: invalid parameters cause y window overflow.")

    window = image[x_start:x_end, y_start:y_end]

    # Zero division errors can occur here
    mean = np.mean(window)
    if mean == 0:
        coef_var = 0.01
    else:
        coef_var = np.std(window) / np.mean(window)
