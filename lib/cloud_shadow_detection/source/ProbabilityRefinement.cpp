    ...
    float DiagonalLength)
{
    ...
    static float const max_distance = 80.f;
    static float const mid_percentile = .2f;
        ...
        for (unsigned int i = influence_bounds.p0.x; i <= influence_bounds.p1.x; i++) {
            for (unsigned int j = influence_bounds.p0.y; j <= influence_bounds.p1.y; j++) {
                ...
                if (!ImOp::at(map, i, j))
                    ...
                    current_distance = 0.f; // No distance since it is a shadow Pixel
    ...
    unsigned int div)
{
    ...
    for (int i = 0; i < alphaMap->size(); i++) {
        ...
        if (mask->data()[i])
            ImOp::set(shadow, cellx, celly, ImOp::at(shadow, cellx, celly) + 1u);
    }
    ...
    for (int i = 0; i < div; i++) {
        for (int j = 0; j < div; j++) {
            if (ImOp::at(total, i, j) > 0u)
                ret.set(i, j, ImOp::at(result, i, j));
        }
    }
}
...
#define VALID(i, j) COUNT(i, j) > 0
    ...
    for (int i = 0; i < D; i++)
        for (int j = 0; j < D; j++)
            if (VALID(i, j))
                ret.set(i, j, (VALUE(i, j) / float(COUNT(i, j))).z);
            else
                empty_pixels.push_back({ i, j });
    ...
    std::shared_ptr<ImageFloat> betaMap)
{
    ...
    static float const W[] = { 16.f / 31.f, 8.f / 31.f, 4.f / 31.f, 2.f / 31.f, 1.f / 31.f };
...
{
    switch (axis) {
    case Bounds::ALPHA_MIN:
        m_alpha_min_clamp = v;
        break;
    case Bounds::ALPHA_MAX:
        m_alpha_max_clamp = v;
        break;
    case Bounds::BETA_MIN:
        m_beta_min_clamp = v;
        break;
    case Bounds::BETA_MAX:
        m_beta_max_clamp = v;
        break;
    }
}
...
void ProbabilityRefinement::UniformProbabilitySurface::clear(Bounds axis)
{
    switch (axis) {
    case Bounds::ALPHA_MIN:
        m_alpha_min_clamp.reset();
        break;
    case Bounds::ALPHA_MAX:
        m_alpha_max_clamp.reset();
        break;
    case Bounds::BETA_MIN:
        m_beta_min_clamp.reset();
        break;
    case Bounds::BETA_MAX:
        m_beta_max_clamp.reset();
        break;
    }
}
