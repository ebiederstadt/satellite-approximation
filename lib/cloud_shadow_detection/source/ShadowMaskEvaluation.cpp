...
std::shared_ptr<ImageUint> GenerateRGBA(std::shared_ptr<ImageUint> A)
{
    ...
    for (int i = 0; i < ret->size(); i++)
        switch (A->data()[i]) {
        case Results::true_negative_class_value:
            ret->data()[i] = TRUE_NEGATIVE_COLOUR;
            break;
        case Results::true_positive_class_value:
            ret->data()[i] = TRUE_POSITIVE_COLOUR;
            break;
        case Results::false_negative_class_value:
            ret->data()[i] = FALSE_NEGATIVE_COLOUR;
            break;
        case Results::false_positive_class_value:
            ret->data()[i] = FALSE_POSITIVE_COLOUR;
            break;
        case Results::clouds_class_value:
            ret->data()[i] = CLOUD_COLOUR;
            break;
        default:
            ret->data()[i] = NO_DATA_COLOUR;
            break;
        }
    return ret;
}
