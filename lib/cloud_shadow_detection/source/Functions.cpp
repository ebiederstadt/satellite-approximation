...
bool Functions::equal(std::string& s1, char const* s2) { return s1.compare(s2) == 0; }
bool Functions::equal(char* s1, std::string& s2) { return s2.compare(s1) == 0; }
bool Functions::equal(char const* s1, std::string& s2) { return s2.compare(s1) == 0; }
...
float Functions::percentile(std::vector<float> collection, float percent)
{
    ...
    return (x < 1) ? 0.f : (x > collection.size()) ? 1.f
                                                   : collection[size_t(x) - 1];
}

float Functions::distance(glm::vec2 p0, glm::vec2 p1)
{
    static float const R = 6371.f; // Radius of the earth in km
    p0 = glm::radians(p0);
    p1 = glm::radians(p1);
...
#define get(i, j) map->operator()(j - p.bounds.p0.y, i - p.bounds.p0.x);
    ...
    for (auto& pix : p.list) {
        ...
        bool edge = (pix.x == p.bounds.p0.x) || (pix.y == p.bounds.p0.y) || (pix.x == p.bounds.p1.x)
            || (pix.y == p.bounds.p1.y);
        if ((!v_up) || (!v_down) || (!v_left) || (!v_right) || edge)
            ret.list.push_back(pix);
    }
    return ret;
}
...
float Functions::quadraticRadialBasis(float d, float min, float max, float percent)
{
    ...
    if (d <= min)
        ...
        return 1.f - (d - min) * (d - min) / ((max - min) * (max - min) * (percent));
    else if (d <= max)
