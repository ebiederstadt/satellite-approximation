...
std::shared_ptr<ImageUint> SceneClassificationLayer::GenerateRGBA(std::shared_ptr<ImageUint> A)
{
    ...
    for (int i = 0; i < ret->size(); i++)
        switch (A->data()[i]) {
        case SATURATED_DEFECTIVE_VALUE:
            ret->data()[i] = SATURATED_DEFECTIVE_COLOUR;
            break;
        case DARK_AREA_PIXELS_VALUE:
            ret->data()[i] = DARK_AREA_PIXELS_COLOUR;
            break;
        case CLOUD_SHADOWS_VALUE:
            ret->data()[i] = CLOUD_SHADOWS_COLOUR;
            break;
        case VEGITATION_VALUE:
            ret->data()[i] = VEGITATION_COLOUR;
            break;
        case BARE_SOIL_VALUE:
            ret->data()[i] = BARE_SOIL_COLOUR;
            break;
        case WATER_VALUE:
            ret->data()[i] = WATER_COLOUR;
            break;
        case CLOUD_LOW_VALUE:
            ret->data()[i] = CLOUD_LOW_COLOUR;
            break;
        case CLOUD_MEDIUM_VALUE:
            ret->data()[i] = CLOUD_MEDIUM_COLOUR;
            break;
        case CLOUD_HIGH_VALUE:
            ret->data()[i] = CLOUD_HIGH_COLOUR;
            break;
        case CLOUD_CIRRUS_VALUE:
            ret->data()[i] = CLOUD_CIRRUS_COLOUR;
            break;
        case SNOW_ICE_VALUE:
            ret->data()[i] = SNOW_ICE_COLOUR;
            break;
        default:
