...
template<class T>
bool valid(Image<T> const& A, size_t i, size_t j)
{
    return ((i >= 0) && (i < A.cols())) && ((j >= 0) && (j < A.rows()));
}
...
template<class T>
T at(Image<T> const& A, size_t i, size_t j)
{
    return A(A.rows() - 1 - j, i);
}
...
template<class T>
void set(Image<T>& A, size_t i, size_t j, T v)
{
    A(A.rows() - 1 - j, i) = v;
}
...
template<class T>
glm::vec2 sides(Image<T> const& A, float Diagonal)
{
    return Diagonal * glm::normalize(glm::vec2(A.cols(), A.rows()));
}
...
template<class T>
glm::ivec2 index(Image<T> const& A, float Diagonal, glm::vec2 pos)
{
    glm::vec2 side_lengths = sides<T>(A, Diagonal);
    return {
        floorf(float(A.cols()) * pos.x / side_lengths.x),
...
template<class T, class Y>
bool DIM_CHECK(Image<T> const& A, Image<Y> const& B)
{
    return (A.rows() == B.rows()) && (A.cols() == B.cols());
}
